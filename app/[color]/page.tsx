...
export default function ColorPage({ params }: { params: { color: string } }) {
  ...
  const [selectedColor, setSelectedColor] = useState(colorMap[params.color] || '#FFFFFF');
  const [customColor, setCustomColor] = useState(colorMap[params.color] || '#FFFFFF');
  const [resolution, setResolution] = useState({ width: 1920, height: 1080 });

  const colorName = params.color.replace('-screen', '').charAt(0).toUpperCase() + params.color.replace('-screen', '').slice(1);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-4xl font-bold mb-8 text-center">{colorName} Screen</h1>
      ...
      <div className="text-center mt-8">
        <Link href="/" className="bg-blue-500 text-white px-6 py-3 rounded-lg hover:bg-blue-600 transition-colors duration-300">
          Back to Color Tool
        </Link>
      </div>
    </div>
